...
public class GanttChart extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        {
            float taskWidth = textBrush.measureText(task.title);
            try {
    ...
    private void drawTimeline(Canvas canvas)
    {
        ...
        while (left <= screenWidth * 2)
        {
            ...
            if (left >= nextLeft && ((oneDayWidth > placeholderDateWidth || (cal.get(Calendar.DATE) == Calendar.SUNDAY && oneDayWidth * 7 <= placeholderDateWidth) || (cal.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY && oneDayWidth * 7 > placeholderDateWidth + 20))))
            {
    ...
    private void drawTasks(Canvas canvas)
    {
        ...
        try {
            for (TaskModel task : tasks)
            {
                ...
                daysOffset = (int) ((task.getStartDate().getTime() - zeroDate.getTime()) / millisecondToDays);

                int daysNumber = (int)((task.getEndDate().getTime() - task.getStartDate().getTime()) / millisecondToDays);
                float left;
                int top, right, bottom;

                if (task.getEndDate().getTime() < zeroDateDisplayed.getTime() && task.links.size() == 0) {
                ...
                if (left < tasklistWidth + tasklistPadding && right < tasklistWidth + tasklistPadding && task.links.size() == 0)
                {
                    //Log.e("GANTT CHART", task.toString() + " skipped!");
                    canvas.drawText((String) TextUtils.ellipsize(task.title, textBrush, tasklistWidth, TextUtils.TruncateAt.END), left, top, textBrush);
                    ++index;
                    continue;
                }
                if (!task.isMilestone && !task.isContainer)
                {
                    float accomplishementWidth = (right - left) * (task.advance / 100);
                ...
                for (Pair<Long, TaskModel.LinkType> pair : task.links)
                {
                    //Pair<Integer, Integer> startPosition = new Pair<Integer, Integer>((int) left, top + (taskHeight / 4));

                    TaskModel endTask = null;
                    int endIndex = 0;
                    for (TaskModel finder : tasks)
                    {
                        if (finder._id == pair.first)
                        {
                            endTask = finder;
                            break;
                        }
    ...
    private ArrayList<Float> drawStartToEndLink(Pair<Integer, Integer> startPosition, int endTaskIndex, int currentTaskIndex, int endTaskRight, TaskModel endTask)
    {
    ...
    private ArrayList<Float> drawEndToEndLink(Pair<Integer, Integer> startPosition, int startTaskDuration, int daysNumber, int currentTaskIndex, int endTaskIndex, TaskModel endTask)
    {
        ArrayList<Float> lines = new ArrayList<>();
    ...
    private ArrayList<Float> drawEndToStartLink(Pair<Integer, Integer> startPosition, int startTaskDuration, int daysNumber, int currentTaskIndex, int endTaskIndex, int endTaskLeft, TaskModel endTask)
    {
        ArrayList<Float> lines = new ArrayList<>();
    ...
    private ArrayList<Float> drawStartToStartLink(Pair<Integer, Integer> startPosition, int currentTaskIndex, int endTaskIndex, int endTaskLeft, TaskModel endTask)
    {
        ArrayList<Float> lines = new ArrayList<>();
        int hauteur  = (startPosition.second + ((taskHeight / 2))) - startPosition.second;
        int baselength = (int) (hauteur / (Math.sqrt(3) / 2));
    ...
    private void drawLink(Canvas canvas, TaskModel.LinkType linkType, Pair<Integer, Integer> startPosition, int startTaskDuration, TaskModel endTask, int endTaskIndex, int currentTaskIndex)
    {
        Path line = new Path();

        int daysOffset = 0;
        try {
            daysOffset = (int) (((endTask.getStartDate().getTime() - zeroDate.getTime()) / millisecondToDays));
            ...
            if (linkType == TaskModel.LinkType.NONE)
                return;
            ...
            switch (linkType)
            {
                case START_TO_END:
                    ...
                    middlePoint = new Pair<>((int) (lines.get(18) + (taskHeight / 2)), (int) ((float) (lines.get(19))));
                ...
                case END_TO_END:
                    ...
                    middlePoint = new Pair<>((int) (lines.get(10) + (taskHeight / 2)), (int) ((float) (lines.get(11))));
                ...
                case START_TO_START:
                    ...
                    middlePoint = new Pair<>((int) (lines.get(10) - (taskHeight / 5)), (int) ((float) (lines.get(11))));
                    endPoint = new Pair<>((int) ((float) (lines.get(10))), (int) ((float) (lines.get(11))));
                    break;
                default:
                    return;
