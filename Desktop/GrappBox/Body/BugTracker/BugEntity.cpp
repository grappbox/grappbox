...
BugEntity::SBugState::SBugState(const int id, const QString &name) : QObject()
{
    _id = id;
    _name = name;
}
...
BugEntity::BugEntity(QJsonObject &obj)
{
    ...
    _state = BugEntity::SBugState(obj["state"].toObject()["id"].toInt(), obj["state"].toObject()["name"].toString());
    if (!obj["tags"].isNull())
    {
...
const BugEntity::SBugState &BugEntity::GetState() const { return _state; }
...
void BugEntity::SetState(const BugEntity::SBugState &state) { _state = state; }
...
void BugEntity::DelTag(QList::const_iterator tagIt) { _tags.removeOne(*tagIt); }
