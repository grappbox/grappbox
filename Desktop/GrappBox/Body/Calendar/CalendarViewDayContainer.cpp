#include "CalendarViewDayContainer.h"

CalendarViewDayContainer::CalendarViewDayContainer(bool viewHour)
{
    setFixedHeight(1920);
...
void CalendarViewDayContainer::paintEvent(QPaintEvent *event)
{
    QPainter paint(this);
    ...
    paint.setPen(QColor(216, 216, 216));
    paint.drawLine(QPointF(rect.width() - 1, 0), QPointF(rect.width() - 1, rect.height() - 1));
...
void CalendarViewDayContainer::LoadEvents(QList<Event *> events, QDate date)
{
...
void CalendarViewDayContainer::HideProject(int id)
{
...
void CalendarViewDayContainer::ShowProject(int id)
{
    for (CalendarEvent *event : _EventsWidget)
    {
...
int CalendarViewDayContainer::GetMaximumColumnForEvent(Event *event, QMap<int, int> eventsOverlap) const
{
...
void CalendarViewDayContainer::LoadEventInterne()
{
