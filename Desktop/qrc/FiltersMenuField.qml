...
Item {
    ...
    readonly property var selectedIndexies: []
    ...
    Component.onCompleted: {
        var realModel = ["All", "None"]
        console.log(realModel)
        console.log(model)
        realModel = realModel.concat(model);
        console.log("AFTER ", realModel)
        listView.model = realModel
        for (var i = 0; i < realModel.length; ++i)
            selectedIndexies.push(i != 1);
    }
    ...
    Item {
        ...
        RowLayout {
            ...
            Label {
                ...
                function changeText()
                {
                    if (selectedIndexies[0])
                        text = "All"
                    else if (selectedIndexies[1])
                        text = "None"
                    else
                    {
                        var nb = 0
                        for (var i = 2; i < selectedIndexies.length; ++i)
                        {
                            if (selectedIndexies[i])
                            {
                                nb++;
                                if (nb > 1)
                                {
                ...
                Component.onCompleted: {
                    itemSelected.connect(changeText)
                }
            }
        ...
        CustomDropdown {
            ...
            ListView {
                ...
                delegate: Standard {
                    ...
                    property bool isActive: selectedIndexies[index]

                    onIsActiveChanged: {
                        selectedIndexies[index] = !check.checked
                        listView.currentIndex = index
                        if (index == 0)
                        {
                            for (var i = 0; i < selectedIndexies.length; ++i)
                                selectedIndexies[i] = true
                        }
                        if (index == 1)
                        {
                            for (var i = 0; i < selectedIndexies.length; ++i)
                                selectedIndexies[i] = false
                        }
                        var isAll = true
                        var isNone = true
                        for (var i = 2; i < selectedIndexies.length; ++i)
                        ...
                        selectedIndexies[0] = isAll;
                        selectedIndexies[1] = isNone;
                        itemSelected(index)
                    }
                    ...
                    action: CheckBox {
                        ...
                        checked: delegateItem.isActive

                        function changed()
                        {
                            check.checked = selectedIndexies[index]
                        }

                        Component.onCompleted: {
                            itemSelected.connect(changed)
                        }

                        onClicked: {
                            delegateItem.isActive = checked
                        }
                    }
                    ...
                    onClicked: {
                        delegateItem.isActive = !delegateItem.isActive
                    }
                }
            }
