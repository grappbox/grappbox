...
class TimelineController extends RolesAndTokenVerificationController
{
	...
	*/
	public function getMessagesAction(Request $request, $token, $id)
	{
		...
		$messages = $em->getRepository('SQLBundle:TimelineMessage')->findBy(array("timelineId" => $timeline->getId(), "deletedAt" => null, "parentId" => null), array("createdAt" => "DESC"));
  ...
  * @apiSuccess {int} parentId parent message id
	...
	*/
	public function getCommentsAction(Request $request, $token, $id, $messageId)
	{
		...
		$messages = $em->getRepository('SQLBundle:TimelineMessage')->findBy(array("timelineId" => $timeline->getId(), "deletedAt" => null, "parentId" => $messageId), array("createdAt" => "ASC"));
		$timelineMessages = array();
		foreach ($messages as $key => $value) {
	...
	*/
	public function getLastMessagesAction(Request $request, $token, $id, $offset, $limit)
	{
		...
		$messages = $em->getRepository('SQLBundle:TimelineMessage')->findBy(array("timelineId" => $timeline->getId(), "deletedAt" => null, "parentId" => null), array("createdAt" => "DESC"), $limit, $offset);
		$timelineMessages = array();
		foreach ($messages as $key => $value) {
			$query = $em->getRepository('SQLBundle:TimelineMessage')->createQueryBuilder('m');
			$commentsNb = $query->select($query->expr()->count('m.id'))
	...
	*/
	public function archiveMessageAction(Request $request, $token, $id, $messageId)
	{
		...
		$comments = $em->getRepository('SQLBundle:TimelineMessage')->findBy(array("parentId" => $message->getId()));
