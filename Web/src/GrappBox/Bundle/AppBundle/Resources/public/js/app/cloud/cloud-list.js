...
*
*/
app.controller("cloudListController", ["$scope", "$http", "$rootScope", function($scope, $http, $rootScope) {
...
var isCloudAccessible = function($q, $http, $rootScope, $route, $location, Notification) {
  var deferred = $q.defer();

  $http.get($rootScope.api.url + "/projects/getinformations/" + $rootScope.user.token + "/" + $route.current.params.id)
...
isCloudAccessible["$inject"] = ["$q", "$http", "$rootScope", "$route", "$location", "Notification"];